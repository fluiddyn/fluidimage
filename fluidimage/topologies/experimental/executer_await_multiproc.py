...
class ExecuterAwaitMultiprocs(ExecuterBase):
                    ...
                    async with cond:
                        while not self.has_to_stop():
                            while not work.func_or_cls(work.input_queue, work.output_queue):
                                ...
                                if self.has_to_stop():
                                    return
                                print("global funtion {} is waiting".format(work.name))
                                await cond.wait()
                                print("global funtion {} is waking up".format(work.name))
                            print("global funtion {} is working".format(work.name))
                            cond.notify_all()
                            print("global funtion {} is waiting".format(work.name))
                            await cond.wait()
                            print("global funtion {} is waking up".format(work.name))
                ...
                async def func(cond, work=w):
                    async with cond:
                        ...
                        while not self.has_to_stop():
                            ...
                            print("global funtion {} is working".format(work.name))
                            key, obj = work.input_queue.queue.popitem()
                            conn = await trio.open_tcp_stream("localhost", port=18813)
                            res = await trio.run_sync_in_worker_thread(work.func_or_cls, obj)
    ...
    def start_server(self):
        ...
        t.start()
        t = ThreadedServer(MyClass, port=18814, protocol_config={"allow_public_attrs": True, "allow_pickle": True})
        t.start()
        t = ThreadedServer(MyClass, port=18815, protocol_config={"allow_public_attrs": True, "allow_pickle": True})
        t.start()

        self.server = rpyc.connect("localhost", 18813, config={"allow_public_attrs": True, "allow_pickle": True})
        self.server = rpyc.connect("localhost", 18814, config={"allow_public_attrs": True, "allow_pickle": True})
        self.server = rpyc.connect("localhost", 18815, config={"allow_public_attrs": True, "allow_pickle": True})
