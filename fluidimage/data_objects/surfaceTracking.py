...
class SurfaceTrackingObject(SerieOfArraysFromFiles):
    ...
    def save(self, nameFrame=None):
        ...
        if self.params.saving.plot == True:
            self.generate_plot(-self.a_filt, nameFrame, 1, self.pix_size*1000, self.pix_size*1000)
    ...
    def save_hdf5(self, nameFrame, path=None):
        ...
        suffix_save = str((math.floor(numFrame/self.params.saving.how_many))+1)
        #Managing path
        if path is not None:
            path_file = path
        else:
        ...
        with h5py.File(path_file + self.file_name +"_"+suffix_save+ ".hdf5", "a") as f:
    ...
    def generate_plot(self, h, name, scale_h=1, scale_x=1, scale_y=1):
        ...
        surf = ax1.plot_surface(x_s, y_s, h_s[:, :], cmap=mycmap,
        ...
        fig.colorbar(surf, shrink=0.6, aspect=40, orientation="horizontal",
                     pad=0.15)
