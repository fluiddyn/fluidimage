...
def pix2phys(calib, X, Y, Zindex):
    ...
    testangle = 0
    if hasattr(calib.slices,'sliceAngle') and np.any(calib.slices.sliceAngle[Zindex] != np.asarray([0,0,0])):
        testangle = 1         
        ...
        Z0 = np.dot(norm_plane,(calib.slices.zsliceCoord[Zindex]))/norm_plane[2];
    else: 
