...
def calib_parameters_from_uvmat(fic):
    ...
    calib_param.f = map(float,np.asarray(get_number_from_string(GeometryCalib['fx_fy'])))
    calib_param.C = np.asarray(get_number_from_string(GeometryCalib['cx__cy']))
    calib_param.kc = np.asarray(GeometryCalib['kc'])
    calib_param.T = np.asarray(get_number_from_string(GeometryCalib['tx__ty__tz']))
  
    
    calib_param.R = [] 
    for i in range(3):
        calib_param.R = np.hstack([calib_param.R, get_number_from_string(GeometryCalib['r_{}'.format(i+1)])])
    ...
    if GeometryCalib['nb_slice'] is not None:
        ...
        for i in range(slices.nbSlice):
            slices.zsliceCoord[i][:] = get_number_from_string( GeometryCalib['slice_coord_{}'.format(i+1)])
        
        if GeometryCalib['slice_angle_1'] is not None:
            slices.sliceAngle = np.zeros([slices.nbSlice,3])
            for i in range (slices.nbSlice):
                slices.sliceAngle[i][:] = get_number_from_string( GeometryCalib['slice_angle_{}'.format(i+1)])
