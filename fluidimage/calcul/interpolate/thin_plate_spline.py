...
def compute_tps_coeff_subdom(centers, U, smoothing_coef, subdom_size, 
    ...
    for i in range(nb_subdomx):
        for j in range(nb_subdomy):
            ...
            (new_positions[1 , :] < y_dom[j+1] + buffer_area_y[j+1]) ).flatten())        
    ...
    for i in range(nb_subdom):
        centerstemp=np.vstack( [centers[0][ind_v_subdom[i]], centers[1][ind_v_subdom[i]] ] )
        Utemp = U[ind_v_subdom[i]]
        U_smooth[i], U_tps[i] = compute_tps_coeff_iter(centerstemp, Utemp, smoothing_coef, threshold)
        
        centers_newposition_temp=np.vstack( [new_positions[0][ind_new_positions_subdom[i]], 
