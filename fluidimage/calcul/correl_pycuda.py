 ...
 #                    numpy.int32,numpy.int32,numpy.int32,numpy.int32],block=(nthreads_x,nthreads_x,nthreads_x))
    ...
    for xiy in range(disp_max + 1):
        ...
        for xix in range(disp_max + 1):
            ...
            blocks_reduce = (blocks_reduce +
                             threads_reduce - 1) // threads_reduce
            correlate_cuda(g_im0data.gpudata, g_im1data.gpudata,
                           g_odata.gpudata, np.int32(nx0), np.int32(nx1),
                           np.int32(ny0), np.int32(dispx), np.int32(dispy),
                           np.int32(nymax), np.int32(ny1dep), np.int32(ny0dep),
                           np.int32(nx0dep), np.int32(nxmax), np.int32(nx1dep),
                           block=(threads_per_block, threads_per_block, 1),
                           grid=(blocks_per_tile, blocks_per_tile))
            while (blocks_reduce != 1):
                reduction_cuda(g_odata.gpudata, g_odata.gpudata,
                ...
                blocks_reduce = (blocks_reduce +
                                 threads_reduce - 1) // threads_reduce
            reduction_cuda(g_odata.gpudata, g_sumdata.gpudata,
                           block=(threads_reduce, 1, 1),
                           grid=(blocks_reduce, 1), shared=4*threads_reduce)
