...
def correl_pythran(im0, im1, disp_max):
    ...
    disp_max => filter image size (im1_crop)

     .. math::
     """
    # correl (\xi_x, \xi_y) = \int_{im0 inter im1} im0*im1_{\delta \xi}
            ...
            for ix in range(nx0 + dispx):
                for iy in range(ny0 - dispy):
                    correl[xiy + disp_max + 1,
                           xix] += im0[iy, ix - dispx] * im1[iy + dispy, ix]
    for xix in range(nx // 2):
        ...
        for xiy in range(ny // 2):
            ...
            for ix in range(nx0 - dispx):
                for iy in range(ny0 - dispy):
                    correl[xiy + disp_max + 1,
                           xix + disp_max + 1] += im0[iy, ix] * im1[iy + dispy,
                                                                    ix + dispx]
