...
class WorkSurfaceTracking(BaseWork):
    ...
    def __init__(self,params):
        ...
        self.gain, self.filt = self.create_gainfilter(self.ky, self.kx, self.k_y, self.k_x, self.l_y, self.l_x, self.slicer)
    ...
    def compute(self,frame):
        ...
        H_sav, H_filt, status, ttlast= self.processAFrame(self.path, self.l_x, self.l_y, self.xmin, self.xmax, self.ymin,
    ...
    def create_gainfilter(self,ky, kx, k_y, k_x, l_y, l_x, slicer):
        ...
        filt1 = np.fft.fftshift(np.exp(-((kxgrid**2 + kygrid**2) /
                                2/(k_x/slicer/l_x)**2))*np.exp(1 -
                                1/(1 + ((kxgrid+k_x)**2 +
                                        (kygrid + k_y)**2)/k_x**2)))
    
        filt2 = np.fft.fftshift(-np.exp(-(((kxgrid+(k_x/l_x))**2 +
                                        kygrid**2)/2/(k_x/10/l_x)**2))+1)
        filt3 = np.fft.fftshift(-np.exp(-(((kxgrid-(k_x/l_x))**2 +
                                        kygrid**2)/2/(k_x/10/l_x)**2))+1)
    ...
    def process_frame(self,frame, ymin, ymax, xmin, xmax, gain, filt, bo, red_factor):
    ...
    def convphase(self,ph, pix_size, l, d, p, correct_pos, red_factor):
        ...
        if correct_pos is True:
            ...
            height = scipy.interpolate.griddata(X+dX, Y+dY, height, X, Y,
                                                'cubic')
        return height

    def wave_vector(self,ref_film, ymin, ymax, xmin, xmax, sur, startref_frame,
                    lastref_frame):
        ...
        for frame in ref_film:
            if ii < lastref_frame-startref_frame:
                ...
                frame1 = self.frame_adimx(frame1)
                ref = ref+frame1
                ref = ref/(lastref_frame+1-startref_frame)  #STRANGE... I think it is not good... BONAMY
            ii += 1
    ...
    def processAFrame(self,path, l_x, l_y, xmin, xmax, ymin, ymax, gain, filt, bo, red_factor,
                pix_size, distance_object, distance_lens, wave_proj,
                n_frames_stock, plot_reduction_factor, kx, ky, frame,save_png=True,
                verify_process=True, filmName=None, offset=0,):
        ...
        H_sav = np.zeros((Ly-8+1, Lx-8+1, n_frames_stock), dtype=np.float32)
        H_filt = np.zeros((Ly-8+1, Lx-8+1, n_frames_stock), dtype=np.float32)
        ...
        iframe = 1
        a2 = self.process_frame(frame, ymin, ymax, xmin, xmax, gain, filt, bo, red_factor)
        if iframe == 1:
            a_mem = a2
            ...
            while(abs(jump) > math.pi):
                ...
                jump = a2[fix_y, fix_x] - a_mem[fix_y, fix_x]
            a_mem = a2
        self.H = self.convphase(a2, pix_size, distance_object, distance_lens,
...
#                             mdict={'H_sav': H_sav, 'H_filt': H_filt})
        ...
        if verify_process:
            ...
            # Plot the surface.
            self.surf = ax.plot_surface(X[0:self.H.shape[0], 0:self.H.shape[1]], Y[0:self.H.shape[0],
                                   0:self.H.shape[1]], self.H_sav[:, :, 0],
                                   linewidth=0, antialiased=False)
            ax = fig.add_subplot(212, projection='3d')
            # Plot the surface.
            self.surf = ax.plot_surface(X[0:self.H.shape[0], 0:self.H.shape[1]], Y[0:self.H.shape[0],
                                   0:self.H.shape[1]], self.H_sav[:, :, 900],
                                   linewidth=0, antialiased=False)
            plt.show()
        
        end_f = datetime.datetime.now()
        ttlast = (end_f-start).seconds + float((end_f-start).microseconds)/1000000
